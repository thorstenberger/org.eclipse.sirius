...
public class RunRepairTest extends AbstractRepairMigrateTest {
    ...
    public void testBlankAird() throws Exception {
        ...
        assertEquals("One error should occur (because the representation files contains no DAnalysis)", 1, problemsListener.getErros().size());
        assertEquals("The error message is not the expected one.", SiriusRepairProcess.ERROR_MSG, problemsListener.getErros().iterator().next().getMessage());
