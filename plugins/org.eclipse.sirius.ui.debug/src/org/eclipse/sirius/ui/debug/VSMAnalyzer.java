...
public class VSMAnalyzer {
    ...
    public String computeStatistics() throws EvaluationException {
        ...
        addStat(" * Number of quence diagrams", "aql:self.eAllContents(description::SequenceDiagramDescription)->size()", result);
    ...
    public String toolSizeDistribution() throws EvaluationException {
        ...
        for (int size : Ordering.natural().sortedCopy(distribution.keySet())) {
            ...
            result.append("| ").append(size).append(" | ").append(distribution.get(size)).append(" | ").append(String.format("%.2f", ((float) acc) / (float) initOperations.size()*100.0)).append(" |\n");
        }
        return result.toString();
    }
